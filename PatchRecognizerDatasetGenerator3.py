...
def GetIntersectionRatio(patchBox, objectBox):
    # for thin objects
    if objectBox[0] > patchBox[0] and objectBox[2] < patchBox[2] and objectBox[1] < patchBox[1] and objectBox[3] > patchBox[3]:
        return 1.0
    elif objectBox[1] > patchBox[1] and objectBox[3] < patchBox[3] and objectBox[0] < patchBox[0] and objectBox[2] > objectBox[2]:
        return 1.0
    intersectionBox = [0] * 4
    ...
    if iFile < trainFiles:
        for i in range(len(newData)):
            ...
            for j in range(len(newData[i])):
                cv2.imwrite(os.path.join(sys.argv[3], 'train', str(i), str(len(trainLines)) + '.png'), newData[i][j])
                trainLines.append('{0}/{1}.png {0}\n'.format(i, len(trainLines)))
    else:
        for i in range(len(newData)):
            if not os.path.exists(os.path.join(sys.argv[3], 'test', str(i))):
                os.makedirs(os.path.join(sys.argv[3], 'test', str(i)))
            for j in range(len(newData[i])):
                cv2.imwrite(os.path.join(sys.argv[3], 'test', str(i), str(len(testLines)) + '.png'), newData[i][j])
                testLines.append('{0}/{1}.png {0}\n'.format(i, len(testLines)))
