...
void LstmLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  // Use a shared Blob with different shape for convinience
...
void LstmLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  for (int t = 0; t < T_; ++t) {
    ...
    // Apply nonlinearity
    caffe_sigmoid(3*N_*H_, pre_i_t, i_t);
    caffe_tanh(N_*H_, pre_g_t, g_t);

    // Compute cell : c(t) = f(t)*c(t-1) + i(t)*g(t)
    const Dtype* c_t_1 = t > 0 ? (c_t - cell_.offset(1)) : c_0_.cpu_data();
    if (clip) {
      caffe_mul(N_*H_, c_t_1, mask_t, clipped_.mutable_cpu_data());
      c_t_1 = clipped_.cpu_data();
    }
    caffe_mul(N_*H_, f_t, c_t_1, c_t);
    caffe_mul(N_*H_, i_t, g_t, ig);
    caffe_add(N_*H_, c_t, ig, c_t);

    // Compute output 
    caffe_tanh(N_*H_, c_t, tanh_c_t);
    caffe_mul(N_*H_, o_t, tanh_c_t, h_t);
  }
...
void LstmLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  for (int t = T_-1; t >= 0; --t) {
    ...
    caffe_mul(N_*H_, tanh_c_t, dh_t, do_t);

    // Cell state : o(t) * tanh'(c(t)) * h_diff(t) + f(t+1) * c_diff(t+1)
    caffe_mul(N_*H_, o_t, dh_t, dc_t);
    caffe_tanh_diff(N_*H_, tanh_c_t, dc_t, dc_t);
    if (t < T_-1) {
      caffe_mul(N_*H_, f_t + gate_.offset(1), dc_t + cell_.offset(1), fdc);
      if (clip) {
        caffe_mul(N_*H_, mask_t + clip_mask_.offset(1), fdc, fdc);
      }
      caffe_add(N_*H_, fdc, dc_t, dc_t);
    }
    ...
    caffe_mul(N_*H_, c_t_1, dc_t, df_t);

    // Input gate : g(t) * c_diff(t)
    caffe_mul(N_*H_, g_t, dc_t, di_t);
    // Input modulation gate : i(t) * c_diff(t)
    caffe_mul(N_*H_, i_t, dc_t, dg_t);

    // Compute derivate before nonlinearity
    caffe_sigmoid_diff(3*N_*H_, i_t, di_t, pre_di_t);
    caffe_tanh_diff(N_*H_, g_t, dg_t, pre_dg_t);

    // Clip deriviates before nonlinearity
    if (clipping_threshold_ > 0.0f) {
      caffe_bound(4*N_*H_, pre_di_t, -clipping_threshold_, 
    ...
    if (t > 0) {
      ...
      caffe_cpu_gemm(CblasNoTrans, CblasNoTrans, N_, H_, 4*H_,
        (Dtype)1., pre_di_t, weight_h, (Dtype)0., clipped_.mutable_cpu_data());
      if (clip) {
        caffe_mul(N_*H_, clipped_.cpu_data(), mask_t, clipped_.mutable_cpu_data());
      }
