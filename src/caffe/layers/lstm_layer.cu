...
void LstmLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  for (int t = 0; t < T_; ++t) {
    ...
    Dtype* f_t = gate_data + gate_.offset(t, 1); 
    Dtype* o_t = gate_data + gate_.offset(t, 2); 
    Dtype* g_t = gate_data + gate_.offset(t, 3); 
    Dtype* pre_i_t = pre_gate_data + pre_gate_.offset(t);
    Dtype* pre_g_t = pre_gate_data + pre_gate_.offset(t, 3);
    ...
    // Apply nonlinearity
    caffe_gpu_sigmoid(3*N_*H_, pre_i_t, i_t);
    caffe_gpu_tanh(N_*H_, pre_g_t, g_t);

    // Compute cell : c(t) = f(t)*c(t-1) + i(t)*g(t)
    const Dtype* c_t_1 = t > 0 ? (c_t - cell_.offset(1)) : c_0_.gpu_data();
    if (clip) {
      caffe_gpu_mul(N_*H_, c_t_1, mask_t, clipped_.mutable_gpu_data());
      c_t_1 = clipped_.gpu_data();
    }
    caffe_gpu_mul(N_*H_, f_t, c_t_1, c_t);
    caffe_gpu_mul(N_*H_, i_t, g_t, ig);
    caffe_gpu_add(N_*H_, c_t, ig, c_t);

    // Compute output 
    caffe_gpu_tanh(N_*H_, c_t, tanh_c_t);
    caffe_gpu_mul(N_*H_, o_t, tanh_c_t, h_t);
  }
...
void LstmLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  for (int t = T_-1; t >= 0; --t) {
    ...
    const Dtype* f_t = gate_data + gate_.offset(t, 1); 
    const Dtype* o_t = gate_data + gate_.offset(t, 2); 
    const Dtype* g_t = gate_data + gate_.offset(t, 3); 
    Dtype* di_t = gate_diff + gate_.offset(t);
    Dtype* df_t = gate_diff + gate_.offset(t, 1); 
    Dtype* do_t = gate_diff + gate_.offset(t, 2); 
    Dtype* dg_t = gate_diff + gate_.offset(t, 3); 
    Dtype* pre_di_t = pre_gate_diff + pre_gate_.offset(t);
    Dtype* pre_dg_t = pre_gate_diff + pre_gate_.offset(t, 3);
    Dtype* fdc = fdc_.mutable_gpu_data();
    const Dtype* mask_t = mask + clip_mask_.offset(t);

    // Output gate : tanh(c(t)) * h_diff(t)
    caffe_gpu_mul(N_*H_, tanh_c_t, dh_t, do_t);

    // Cell state : o(t) * tanh'(c(t)) * h_diff(t) + f(t+1) * c_diff(t+1)
    caffe_gpu_mul(N_*H_, o_t, dh_t, dc_t);
    caffe_gpu_tanh_diff(N_*H_, tanh_c_t, dc_t, dc_t);
    if (t < T_-1) {
      caffe_gpu_mul(N_*H_, f_t + gate_.offset(1), dc_t + cell_.offset(1), fdc);
      if (clip) {
        caffe_gpu_mul(N_*H_, mask_t + clip_mask_.offset(1), fdc, fdc);
      }
      caffe_gpu_add(N_*H_, fdc, dc_t, dc_t);
    }
    ...
    caffe_gpu_mul(N_*H_, c_t_1, dc_t, df_t);

    // Input gate : g(t) * c_diff(t)
    caffe_gpu_mul(N_*H_, g_t, dc_t, di_t);
    // Input modulation gate : i(t) * c_diff(t)
    caffe_gpu_mul(N_*H_, i_t, dc_t, dg_t);

    // Compute derivate before nonlinearity
    caffe_gpu_sigmoid_diff(3*N_*H_, i_t, di_t, pre_di_t);
    caffe_gpu_tanh_diff(N_*H_, g_t, dg_t, pre_dg_t);

    // Clip deriviates before nonlinearity
    if (clipping_threshold_ > 0.0f) {
      caffe_gpu_bound(4*N_*H_, pre_di_t, -clipping_threshold_, 
          clipping_threshold_, pre_di_t);
    }
    
    if (t > 0) {
      Dtype* dh_t_1 = dh_t - top_.offset(1);
      // Backprop output errors to the previous time step
      caffe_gpu_gemm(CblasNoTrans, CblasNoTrans, N_, H_, 4*H_,
        (Dtype)1., pre_di_t, weight_h, (Dtype)0., clipped_.mutable_gpu_data());
      if (clip) {
        caffe_gpu_mul(N_*H_, clipped_.gpu_data(), mask_t, clipped_.mutable_gpu_data());
      }
