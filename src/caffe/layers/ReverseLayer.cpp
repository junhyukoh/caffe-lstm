	...
	const vector<Blob<Dtype>*>& top )
{
	...
	for( int t = 0; t < T_; ++t ) {
		const int bottom_offset = bottom[0]->offset( t );
		const int top_offset = top[0]->offset( T_ - t - 1 );
		caffe_copy( bottom[0]->count( 1 ), bottom_data[bottom_offset], top_data[top_offset] );
	}
}
	...
	const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom )
{
	...
	for( int t = 0; t < T_; ++t ) {
		const int bottom_offset = bottom[0]->offset( t );
		const int top_offset = top[0]->offset( T_ - t - 1 );
		caffe_copy( bottom[0]->count( 1 ), top_diff[top_offset], bottom_diff[bottom_offset] );
	}
}
